...
InModuleScope $ElasticModule {
    Describe "Get-ElasticSearch Unit Tests" -Tag 'Unit' {
        Context "$ElasticFunction return value validation" {
            ...
            $mockLocation = "$ElasticMockDataLocation\Get-ElasticSearch.Mock"
            ...
            # Act
            $result = Get-ElasticSearch -Name 'mock' -Query 'query'
