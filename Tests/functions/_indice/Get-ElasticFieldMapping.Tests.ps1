...
InModuleScope $ElasticModule {
    Describe "Get-ElasticFieldMapping Unit Tests" -Tag 'Unit' {
        Context "$ElasticFunction return value validation" {
            ...
            $mockLocation = "$ElasticMockDataLocation\Get-ElasticFieldMapping.Mock"
            ...
            # Act
            $result = Get-ElasticFieldMapping -Field 'test'
